...
const CSV_COLS = [
  ...
  ["intelligence_aa", (r) => r.intel], ["intelligence_aa_index_version", (r) => r.intelVersion],
  ["coding_agent_aa", (r) => r.codingAgent], ["coding_agent_harness", (r) => r.codingAgentVia],
  ["coding_agent_aa_index_version", (r) => r.codingAgentVersion],
  ["agentic_aa", (r) => r.agentic], ["vision_mmmu_pro_aa", (r) => r.vision],
...
const schema = {
  ...
  fields: {
    ...
    intel: "Artificial Analysis Intelligence Index. Third-party. Read intelVersion before comparing two models: the column holds figures from two versions of the index.",
    intelVersion: "The version of the AA Intelligence Index the intel figure was measured under. AA re-based the index at v4.2 and again at v4.3 by adding harder evaluations rather than re-testing, which moved every score down 12-18 points, and it has re-rated only some of these models. A '4.1' figure and a '4.3' figure are not on the same scale and must not be compared, ranked or averaged together. null where intel is null.",
    codingAgent: "Artificial Analysis Coding Agent Index. Third-party. Scores an agent harness driving the model, not the model alone, so it is only meaningful together with codingAgentVia. Read codingAgentVersion before comparing two models: the column holds figures from two versions of the index.",
    codingAgentVersion: "The version of the AA Coding Agent Index the codingAgent figure was measured under. AA re-based it from v1.3 to v1.5 by swapping Terminal-Bench v2 for v4 and DeepSWE for v1.1, which moved every re-rated pairing down 7-16 points, and it has re-rated only some of these models. A '1.3' figure and a '1.5' figure are not on the same scale and must not be compared, ranked or averaged together. null where codingAgent is null.",
    codingAgentVia: "The agent harness that produced codingAgent — the highest-scoring pairing AA publishes for this model.",
    vision: "MMMU-Pro percentage as published by Artificial Analysis. Third-party, and a single benchmark rather than an index — none of intel, codingAgent or agentic includes a vision evaluation, so a null here is common on models that lead those.",
